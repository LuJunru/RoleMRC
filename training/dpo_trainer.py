...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        lnorm_loc: Literal["logps", "logrs"] = "logrs",
        lnorm_type: Literal["scale", "top", "random"] = "scale",
        dpop_lambda: float = 50.0,
        tdpo_alpha: float = 0.5,
        sft_lambda: float = 0.05,
        l1_lambda: float = 0.1,
        add_noise: Optional[bool] = False,
        neft_alpha: Optional[float] = 5.0,
        sampling_distribution: str = "multinomial",
    ):
        ...
        if loss_type in ["hinge", "ipo", "kto_pair", "dpop", "mix", "l1", "tdpo", "bco_pair", "sppo_hard", "nca_pair"] and label_smoothing > 0:
            warnings.warn(
    ...
    def compute_reference_log_probs(self, padded_batch: Dict, type_label=None) -> Dict:
    ...
    def dpo_loss(
        ...
        if self.len_norm:
            if self.lnorm_type == "scale":
                # SANorm, however logps sanorm <==> logrs sanorm
            ...
            elif self.lnorm_type == "top":
                # TopKNorm
                if self.lnorm_loc == "logps":
                    k_largest = False  # since logps are always negative
                else:
                    k_largest = True  # logrs could be positive
                batch_size = cho_logratios.size(0)
                topk_cho_logratios = torch.zeros(batch_size, device=cho_logratios.device)
                topk_rej_logratios = torch.zeros(batch_size, device=rej_logratios.device)
                for i in range(batch_size):
                    if self.rejected_loss_mask[i].sum(-1) < self.chosen_loss_mask[i].sum(-1):
                        min_length = self.rejected_loss_mask[i].sum(-1).item()
                        if self.lnorm_loc == "logps":
                            topk_cho_logratios[i] = torch.topk((policy_chosen_logps[i] * self.chosen_loss_mask[i])[self.chosen_loss_mask[i] != 0], 
                                                                k=min_length, dim=0, largest=k_largest)[0].sum(-1) - \
                                                    torch.topk((reference_chosen_logps[i] * self.chosen_loss_mask[i])[self.chosen_loss_mask[i] != 0], 
                                                                k=min_length, dim=0, largest=k_largest)[0].sum(-1)
                            topk_rej_logratios[i] = (policy_rejected_logps[i] * self.rejected_loss_mask[i]).sum(-1) - \
                                                    (reference_rejected_logps[i] * self.rejected_loss_mask[i]).sum(-1)
                        else:
                            topk_cho_logratios[i] = torch.topk((cho_logratios[i] * self.chosen_loss_mask[i])[self.chosen_loss_mask[i] != 0], 
                                                                k=min_length, dim=0, largest=k_largest)[0].sum(-1)
                            topk_rej_logratios[i] = (rej_logratios[i] * self.rejected_loss_mask[i]).sum(-1)
                    else:
                        min_length = self.chosen_loss_mask[i].sum(-1).item()
                        if self.lnorm_loc == "logps":
                            topk_cho_logratios[i] = (policy_chosen_logps[i] * self.chosen_loss_mask[i]).sum(-1) - \
                                                    (reference_chosen_logps[i] * self.chosen_loss_mask[i]).sum(-1)
                            topk_rej_logratios[i] = torch.topk((policy_rejected_logps[i] * self.rejected_loss_mask[i])[self.rejected_loss_mask[i] != 0], 
                                                                k=min_length, dim=0, largest=k_largest)[0].sum(-1) - \
                                                    torch.topk((reference_rejected_logps[i] * self.rejected_loss_mask[i])[self.rejected_loss_mask[i] != 0], 
                                                                k=min_length, dim=0, largest=k_largest)[0].sum(-1)
                        else:
                            topk_cho_logratios[i] = (cho_logratios[i] * self.chosen_loss_mask[i]).sum(-1)
                            topk_rej_logratios[i] = torch.topk((rej_logratios[i] * self.rejected_loss_mask[i])[self.rejected_loss_mask[i] != 0], 
                                                                k=min_length, dim=0, largest=k_largest)[0].sum(-1)
                del cho_logratios
                del rej_logratios
                cho_logratios = topk_cho_logratios
                rej_logratios = topk_rej_logratios
                del topk_cho_logratios
                del topk_rej_logratios
            elif self.lnorm_type == "random":
                # RandomKNorm
                batch_size = cho_logratios.size(0)
                ...
                for i in range(batch_size):
                    if self.rejected_loss_mask[i].sum(-1) < self.chosen_loss_mask[i].sum(-1):
                        ...
                        if self.lnorm_loc == "logps":
                            ...
                            random_cho_logratios[i] = (cho_logratios[i] * self.chosen_loss_mask[i])[chosen_indices_1].sum(-1)
                            random_rej_logratios[i] = (rej_logratios[i] * self.rejected_loss_mask[i]).sum(-1)
                    else:
                        min_length = self.chosen_loss_mask[i].sum(-1).item()
                        ...
                        if self.lnorm_loc == "logps":
                            ...
                            random_cho_logratios[i] = (cho_logratios[i] * self.chosen_loss_mask[i]).sum(-1)
                            random_rej_logratios[i] = (rej_logratios[i] * self.rejected_loss_mask[i])[rejected_indices_1].sum(-1)
                del cho_logratios
                del rej_logratios
        ...
        if self.loss_type == "sigmoid":
            ...
            # dpop: https://github.com/abacusai/smaug/issues/2#issuecomment-2075172150
            ...
            losses = (
                -F.logsigmoid(self.beta * logits + self.lenN_lambda * (self.chosen_loss_mask.sum(-1) - self.rejected_loss_mask.sum(-1)))
            )
            ...
            raise ValueError(
                f"Unknown loss type: {self.loss_type}. Should be one of ['sigmoid', 'hinge', 'ipo', 'kto_pair', 'dpop', 'mix', 'lenN', 'tdpo', 'bco_pair', 'sppo_hard', 'nca_pair']"
            )
    ...
    def tdpo_loss(
        ...
        logits = chosen_rejected_logps_margin - self.tdpo_alpha * (rejected_position_kl - chosen_position_kl.detach())  # tdpo2
    ...
    def tdpo_get_batch_logps(
        ...
        Returns:
            A tensor of shape (batch_size,) containing the average/sum log probabilities of the given labels under the given logits.
        """
        if logits.shape[:-1] != labels.shape or reference_logits.shape[:-1] != labels.shape:
            raise ValueError("Logits (batch and sequence length dim) and labels must have the same shape.")
    ...
    def concatenated_forward(
        self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]], add_noise=False, keep_org_logps=False
        ...
        if add_noise:
            # linear increase alpha from 5.0 to 15.0, since larger alpha represents harder negative samples (implied by the NEFT paper)
            self.neftune_noise_alpha = self.neft_alpha + self.state.global_step / self.state.max_steps * 10.0
        ...
        if add_noise:
            self._deactivate_neftune(model)
        ...
        chosen_loss_mask = all_loss_mask[:len_chosen]
        rejected_loss_mask = all_loss_mask[len_chosen:]
    ...
    def tdpo_concatenated_forward(
        self, model: nn.Module, reference_model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]]
    ):
        ...
        all_logps_margin, all_position_kl, all_logps = self.tdpo_get_batch_logps(
            all_logits, 
        ...
        chosen_logps_margin = all_logps_margin[:len_chosen]
        rejected_logps_margin = all_logps_margin[len_chosen:]
        chosen_position_kl = all_position_kl[:len_chosen]
        rejected_position_kl = all_position_kl[len_chosen:]

        chosen_logps = all_logps[:len_chosen].detach()
        rejected_logps = all_logps[len_chosen:].detach()

        return (chosen_logps_margin, rejected_logps_margin, chosen_position_kl, rejected_position_kl, chosen_logps, rejected_logps)
    ...
    def get_batch_loss_metrics(
        ...
        if self.loss_type == "tdpo":
            """Compute the TDPO loss and other metrics for the given batch of inputs for train or test."""
            ( 
                chosen_logps_margin, 
            ...
            ) = self.tdpo_concatenated_forward(model, self.ref_model, batch)
            ...
            ) = self.concatenated_forward(model, batch, add_noise=True if self.add_noise is True else False, keep_org_logps=True)
